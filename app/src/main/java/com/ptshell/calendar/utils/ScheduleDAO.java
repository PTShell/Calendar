package com.ptshell.calendar.utils;
...
public class ScheduleDAO {
     ...
     * @param scheduleVO
     */
    public int save(ScheduleBean scheduleVO) {
    ...
    public ScheduleBean getScheduleByID(int scheduleID) {
        ...
        if (cursor.moveToFirst()) {
            ...
            cursor.close();
            return new ScheduleBean(schID, scheduleTypeID, remindID, scheduleContent, scheduleDate);
        }
        cursor.close();
        return null;
    ...
    public ArrayList<ScheduleBean> getAllSchedule() {
        ...
        while (cursor.moveToNext()) {
            ...
            String scheduleContent = cursor.getString(cursor.getColumnIndex("scheduleContent"));
            String scheduleDate = cursor.getString(cursor.getColumnIndex("scheduleDate"));
            ScheduleBean vo = new ScheduleBean(scheduleID, scheduleTypeID, remindID, scheduleContent, scheduleDate);
            list.add(vo);
        }
        cursor.close();
     ...
     * @param vo
     */
    public void update(ScheduleBean vo) {
        //dbOpenHelper = new DBOpenHelper(context, "schedules.db");
        SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
     ...
     * @param dateTagList
     */
    public void saveTagDate(ArrayList<ScheduleDateBean> dateTagList) {
        //dbOpenHelper = new DBOpenHelper(context, "schedules.db");
        SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
     ...
     * @return
     */
    public ArrayList<ScheduleDateBean> getTagDate(int currentYear, int currentMonth) {
        ArrayList<ScheduleDateBean> dateTagList = new ArrayList<ScheduleDateBean>();
        //dbOpenHelper = new DBOpenHelper(context, "schedules.db");
        SQLiteDatabase db = dbOpenHelper.getReadableDatabase();
        ...
        while (cursor.moveToNext()) {
            ...
            int scheduleID = cursor.getInt(cursor.getColumnIndex("scheduleID"));
            ScheduleDateBean dateTag = new ScheduleDateBean(tagID, year, month, day, scheduleID);
